...
local C = {
    ...
    Misc = {AntiAFK=true, Gacha=false, Collect=true, AntiBan=true, AntiAdmin=true,
            SeaTravel=false, HopWhenAdmin=true, WhiteScreen=false, CloudRemotes=false,
            BruteForce=false, HookCheck=true, RubberDetect=false},
    Spy = false,
    Manual = {}
}
...
local function ScanRemotes()
    ...
    local function sc(f,d)
        ...
        for _,o in pairs(f:GetChildren()) do
            ...
            if o:IsA("Folder") or o:IsA("Configuration") or o:IsA("ScreenGui") then sc(o,d+1) end end end end
    sc(S.RS,0)
end
ScanRemotes()
...
-- === BACKGROUND LOOPS (each wrapped in pcall) ===
if C.Misc.AntiAFK then
    LP.Idled:Connect(function()
        pcall(function()
            S.VU:Button2Down(Vector2.new(0,0),Camera.CFrame); task.wait(1)
            S.VU:Button2Up(Vector2.new(0,0),Camera.CFrame) end) end)
end

S.UIS.JumpRequest:Connect(function()
    pcall(function()
        if C.Move.InfJump then local r=HRP(); if r then r.Velocity=Vector3.new(r.Velocity.X,C.Move.JumpP,r.Velocity.Z) end end end) end)

task.spawn(function() pcall(function() while task.wait(0.2) do
    if C.Move.AntiFall and IsAlive() then local r=HRP(); if r and r.Position.Y<-80 then r.CFrame=CFrame.new(r.Position.X,80,r.Position.Z) end end end end) end)

task.spawn(function() pcall(function() while task.wait(0.1) do
    if C.Move.SpdOn and IsAlive() then local h=Hum(); if h then h.WalkSpeed=C.Move.Spd end end end end) end)

task.spawn(function() pcall(function() while task.wait(4) do if C.Player.AutoStat then AutoStat() end end end) end)

task.spawn(function() pcall(function() while task.wait(15) do if C.Player.AutoHaki then AutoHaki() end end end) end)

task.spawn(function() pcall(function() while task.wait(8) do if C.Misc.Gacha then local r=R("Gacha"); if r then for _,rr in ipairs(r) do pcall(function() rr:InvokeServer("Buy Fruit") end) end end end end end) end)

task.spawn(function() pcall(function() while task.wait(1) do
    ...
    if C.ESP.On then
        ...
        for _,o in pairs(S.WS:GetDescendants()) do
            ...
            if C.ESP.Chests and o:IsA("BasePart") and o.Name:find("Chest") then ESPApply(o, Color3.new(1,1,0)) end end end end end) end)

-- === MAIN LOOP (no 'continue', using if-blocks for compatibility) ===
local function FarmLoop()
    while task.wait(C.Farm.AtkSpd) do
        if not C.Farm.Enabled then task.wait(0.5)
        elseif not IsAlive() then task.wait(3)
        else
            local me = HRP()
            if me then
                local zone = GetZone()
                if zone then
                    pcall(Collect)
                    pcall(CollectMats)
                    if C.Farm.AutoStore then pcall(AutoStore) end
                    if C.Farm.AutoRaid then pcall(RaidFunc) end
                    if C.Misc.SeaTravel then pcall(SeaTravelFunc) end

                    pcall(function() ScanMobs(zone.Mobs, C.Farm.AutoBoss) end)
                    pcall(TrackKills)
                    ...
                    if #Targets==0 then
                        QuestActive = false
                        if C.Farm.QuestMode=="Auto" then pcall(AcceptQuest)
                        elseif C.Move.Bypass then
                            local d = (me.Position-zone.Pos).Magnitude
                            if d>80 then pcall(BypassTween, CFrame.new(zone.Pos+Vector3.new(0,C.Farm.SafeY,0))) end end
                    else
                        ...
                        if d>8 then
                            ...
                            if C.Move.VelMode then pcall(VelocityMove, hov.Position)
                            elseif C.Move.Bypass then pcall(BypassTween, hov)
                            else pcall(function() local r=HRP(); if r then r.CFrame=hov end end) end
                        else
                            me.Velocity = Vector3.new(0,0,0)
                            if C.Farm.Bring then pcall(BringAll, me, zone.Mobs, C.Farm.AutoBoss) end

                            local distCheck = (me.Position-target.HRP.Position).Magnitude
                            if C.Combat.DistanceCheck and distCheck>25 then
                                if C.Move.Bypass then pcall(BypassTween, target.HRP.CFrame*CFrame.new(0,C.Farm.SafeY,0)) end
                            else
                                ...
                                if C.Farm.FastAtk and cRemote then
                                    ...
                                    for atk=1,atkCount do
                                        ...
                                        local delay = C.Move.AdaptiveAtk and pcall(AdaptiveDelay, C.Farm.AtkSpd) or C.Farm.AtkSpd
                                        task.wait(type(delay)=="number" and delay or C.Farm.AtkSpd)
                                    end
                                    pcall(UseSkills, target)
                                else
                                    if tool then tool:Activate() end
                                    if cRemote then
                                        pcall(function()
...
local function Init()
    ...
    local ok1, ok2, ok3
    ok1 = pcall(FindLevel)
    ok2 = pcall(BruteForceRemotes)
    ok3 = pcall(InitHooks)
    for cat, name in pairs(C.Manual) do pcall(SetManualRemote, cat, name) end
    task.spawn(FarmLoop)
    local guiOk, guiErr = pcall(LoadGUI)
    if not guiOk then
        warn("[QF] Fluent GUI failed:", guiErr)
        pcall(BuildFallbackGUI)
    end
    print("=== QUANTUM FORGE+ v2 ACTIVE ===")
    print("Init: Find="..tostring(ok1).." Brute="..tostring(ok2).." Hooks="..tostring(ok3).." GUI="..tostring(guiOk))
    print("Fast Atk+Adaptive | Velocity Move | Map Bring | Sea Travel | Raid | Kill Track")
    pcall(PrintRemotes)
end

local ok, err = xpcall(Init, function(e)
    warn("[QF] FATAL INIT ERROR:", e)
    local tb = pcall(function() return debug.traceback() end)
    if tb then print(tb) end
end)
